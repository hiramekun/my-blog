...
**種別**: 静的サイトジェネレーター型ブログ  
**ターゲットプラットフォーム**: Cloudflare Pages（`blog.hiramekun.dev`）  
**主要言語**: 日本語（ドキュメントは英語対応）  

> この仕様書が対象とするのはモノレポのうち `apps/blog` のみ。
> `hiramekun.dev`（プロフィール / リンク集）は `apps/profile` にあり、対象外。
...
### ユーザーインターフェース
- **FR-017**: Material Design 3 のライト／ダークテーマ（tech-notes と共通のトークン）
...
- **TR-004**: Cloudflare Pages への Git 連携デプロイメント（`main` への push で自動）

### 開発スタック
- **TR-005**: コンポーネントアーキテクチャのためのReact 18+
- **TR-006**: カスタムCSS変数を使用したTailwind CSS
- **TR-007**: Markdown処理のための`react-markdown`（XSS 安全なレンダリング）
- **TR-008**: シンタックスハイライトのための`rehype-highlight` + `highlight.js`
- **TR-009**: フロントマター解析のための`gray-matter`
- **TR-010**: GFM サポートのための`remark-gfm`
...
### ビジュアルデザイン
- **DR-001**: プライマリデザイン言語としての Material Design 3（ライト／ダーク両対応）
...
### ファイル構成
```
posts/                      # リポジトリ直下（Obsidian の vault）
├── first-post.md
...
└── lib/
    ...
    ├── metadata.ts         # メタデータ生成
    └── sites.ts            # hiramekun.dev 配下のサイト URL
```
...
- **IR-001**: Terraform（`infra/`）で Pages プロジェクトと DNS を管理
- **IR-002**: Git 連携で `main` を本番ブランチに設定
- **IR-003**: カスタムドメイン `blog.hiramekun.dev`
- **IR-004**: HTTPS強制有効化（Cloudflare が自動で行う）
...
3. **UI開発**: Material Design 3 によるレスポンシブコンポーネントの作成
...
この仕様書は、優れたユーザーエクスペリエンスと開発者ワークフローを維持しながらCloudflare Pages デプロイメントに最適化された、モダンで効率的な個人ブログプラットフォーム構築のための包括的な基盤を提供します。