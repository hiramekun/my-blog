...
このブログは**はてなブログ**のシンプルで読みやすいデザインを参考にしています：

- **2カラムレイアウト**: メインコンテンツ + 機能的なサイドバー
- **ミニマルな色使い**: 白背景、濃いグレー文字、控えめなアクセント
...
- **基本スタイリング**: `src/app/globals.css` でマークダウン表示やカスタムスタイルを調整
- **コンポーネントスタイル**: 各`.tsx`ファイルのTailwind CSSクラスを変更
